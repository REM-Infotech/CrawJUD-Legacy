...
from crawjud_bots.web.decorators import current_user, login_required
...
async def serve_profile(user: str) -> Response:
    ...
    try:
        with app.app_context():
            ...
            original_path = os.path.join(crawjud_bots.app.config["IMAGE_TEMP_DIR"], filename)

            with aiofiles.open(original_path, "wb") as file:
                file.write(image_data)

            response = await make_response(
                await send_from_directory(crawjud_bots.app.config["IMAGE_TEMP_DIR"], filename)
            )
